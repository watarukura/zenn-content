...
一方で、他のユーザへはメール送信処理を継続し、最後にエラーになったメールアドレスについて通知して、何らかの方法で対応することにします。
...
1000件ごとにひとかたまり(=チャンク)にして、メール送信APIを呼び出すようにしてみます。
...
件数が増えてきた場合、かつ、ある程度の時間の間に宛先にメールを届けたい場合にどうするか、ということになります。  
